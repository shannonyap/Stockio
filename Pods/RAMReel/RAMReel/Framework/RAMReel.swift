    ...
    public init(frame: CGRect, dataSource: DataSource, placeholder: String = "", hook: HookType? = nil) {
        ...
        returnTarget.beTargetFor(textField, controlEvents: controlEvents) { textField -> () in
            if
                ...
                var item = DataSource.ResultType.parse(text)
            {
                if self.wrapper.selectedItem != nil {
                    item = DataSource.ResultType.parse((self.wrapper.selectedItem?.render())!)!
                }
                
                if textField.text != self.wrapper.selectedItem?.render() && self.wrapper.selectedItem != nil {
                    textField.text = self.wrapper.selectedItem?.render()
                } else {
                    textField.text = text
                }
